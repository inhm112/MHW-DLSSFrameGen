...
namespace
{
bool IsMhwFgProductionPathAnchorActive()
{
#ifdef MHWFG_PRODUCTION
    return Util::ToLower(Util::ExePath().filename().wstring()) == L"monsterhunterworld.exe";
#else
    return false;
#endif
}

bool IsOrdinaryRelativeConfigPath(const std::filesystem::path& path)
{
    if (path.empty() || path.is_absolute() || !path.is_relative())
        return false;

    // Drive-relative paths (e.g. C:folder) are not ordinary relatives for this contract.
    if (path.has_root_name())
        return false;

    // Root-relative paths (e.g. \OptiScaler\file.dll or /OptiScaler) are not ordinary relatives.
...
void AnchorMhwFgDlssConfigPath(CustomOptional<std::wstring, NoDefault>& setting)
{
    if (!setting.has_value())
        return;

    const std::filesystem::path configPath(setting.value());
    if (!IsOrdinaryRelativeConfigPath(configPath))
        return;

    setting.set_volatile_value(
        std::filesystem::absolute(Util::ExePath().parent_path() / configPath).wstring());
}

void AnchorMhwFgProductionDlssPaths()
{
    if (!IsMhwFgProductionPathAnchorActive())
        return;

    auto* config = Config::Instance();
    AnchorMhwFgDlssConfigPath(config->NVNGX_DLSS_Library);
    AnchorMhwFgDlssConfigPath(config->DLSSFeaturePath);
}
} // namespace
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved)
{
    ...
    case DLL_PROCESS_ATTACH:
    {
        ...
        AnchorMhwFgProductionDlssPaths();
