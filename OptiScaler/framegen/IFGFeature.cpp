...
UINT64 IFGFeature::StartNewFrame()
{
    ...
    {
        using FrameJumpClock = std::chrono::steady_clock;
        static thread_local FrameJumpClock::time_point lastWarn {};
        static thread_local uint32_t suppressed = 0;
        const auto now = FrameJumpClock::now();
        if (lastWarn == FrameJumpClock::time_point {} || now - lastWarn >= std::chrono::seconds(5))
        {
            if (suppressed > 0)
                LOG_WARN("Frame count jumped {} times in 5s; last _frameCount: {}, _lastDispatchedFrame: {}",
                         suppressed + 1, _frameCount, _lastDispatchedFrame);
            else
