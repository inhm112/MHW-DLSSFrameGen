...
static bool MhwDlssBackend(Upscaler upscaler) { return upscaler == Upscaler::DLSS || upscaler == Upscaler::DLSSD; }

static void ReleaseCreateParams(ContextData<IFeature_Dx12>& ctx)
{
    if (ctx.createParams == nullptr)
        return;

    if (ctx.createParamsOwned)
        TryDestroyNGXParameters(ctx.createParams, NVNGXProxy::D3D12_DestroyParameters());

    ctx.createParams = nullptr;
    ctx.createParamsOwned = false;
}
                                             ...
                                             NVSDK_NGX_Parameter* InParameters, NVSDK_NGX_Handle** OutHandle)
{
    ...
    if (!FeatureProvider_Dx12::GetFeature(upscalerBackend, handleId, InParameters, &Dx12Contexts[handleId].feature))
    {
        ...
        if (MhwFgFailure::PolicyActive() && MhwDlssBackend(upscalerBackend))
            MhwFgFailure::Latch(MhwFgFailureStage::FeatureCreate);
        ...
        Util::DelayedDestroy(std::move(Dx12Contexts[handleId].feature));
    ...
    if (!EnsureD3D12Device(InCmdList))
    {
        ...
        if (MhwFgFailure::PolicyActive() && MhwDlssBackend(upscalerBackend))
        {
            Dx12Contexts[handleId].terminalFailure = true;
            MhwFgFailure::Latch(MhwFgFailureStage::FeatureCreate);
        }
    ...
    // Assign handle
    const bool outHandleWasNull = (*OutHandle == nullptr);
    const uint32_t previousHandleId = outHandleWasNull ? 0u : (*OutHandle)->Id;
    ...
    else
    {
        ...
        {
            Dx12Contexts[handleId].terminalFailure = true;
            MhwFgFailure::Latch(MhwFgFailureStage::InitFailed);
                                               ...
                                               PFN_NVSDK_NGX_ProgressCallback InCallback)
{
    ...
    if (feature != nullptr && !feature->IsInited() && cfg.Dx12Upscaler.value_or_default() != Upscaler::FSR21)
    {
        if (MhwFgFailure::PolicyActive() && MhwDlssBackend(feature->GetUpscalerType()))
        {
            ctxData.terminalFailure = true;
            MhwFgFailure::Latch(MhwFgFailureStage::InitFailed);
