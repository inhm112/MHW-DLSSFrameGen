...
void ResTrack_Dx12::hkExecuteCommandLists(ID3D12CommandQueue* This, UINT NumCommandLists,
                                          ID3D12CommandList* const* ppCommandLists)
{
    const bool c048Diag = DLSSG_Dx12::C048DiagEnabled();
    ...
    if (fg != nullptr && fg->IsActive() && !fg->IsPaused())
    {
        ...
        if (!found.empty())
        {
            const uint32_t c048Token =
                c048Diag ? DLSSG_Dx12::C048DiagPrepareExecuteCommandLists(This, NumCommandLists, ppCommandLists) : 0;

            const uint64_t c048OriginalEntryUs = c048Diag ? DLSSG_Dx12::C048DiagNowUs() : 0;
            o_ExecuteCommandLists(This, NumCommandLists, ppCommandLists);
            const uint64_t c048OriginalExitUs = c048Diag ? DLSSG_Dx12::C048DiagNowUs() : 0;

            if (c048Diag)
                DLSSG_Dx12::C048DiagCompleteExecuteCommandLists(c048Token, This, c048OriginalEntryUs,
                                                                c048OriginalExitUs);
            ...
            if (c048Diag)
                DLSSG_Dx12::C048DiagMaybeReport();
    ...
    if (c048Diag)
    {
        DLSSG_Dx12::C048DiagCompleteExecuteCommandLists(c048Token, This, c048OriginalEntryUs, c048OriginalExitUs);
        DLSSG_Dx12::C048DiagMaybeReport();
    }
}

bool ResTrack_Dx12::IsExecuteCommandListsHookInstalled() { return o_ExecuteCommandLists != nullptr; }
