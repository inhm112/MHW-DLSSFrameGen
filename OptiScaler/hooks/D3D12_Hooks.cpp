...
enum class C072EntryPoint : uint8_t
{
    D3D12CreateDevice,
    DeviceFactoryCreateDevice,
};

static std::once_flag g_c072EarlyDebugLayerOnce;

static bool C072DiagEnabledImpl()
{
    static const bool enabled = []
    {
        ...
        char envValue[8] = {};
        const DWORD envLen =
            GetEnvironmentVariableA("MHWFG_DIAG_EARLY_DEBUG_LAYER", envValue, static_cast<DWORD>(sizeof(envValue)));
        return envLen == 1 && envValue[0] == '1' && envValue[1] == '\0';
    }();
    return enabled;
}

static const char* C072EntryPointName(C072EntryPoint entry)
{
    switch (entry)
    {
    case C072EntryPoint::D3D12CreateDevice:
        return "D3D12CreateDevice";
    case C072EntryPoint::DeviceFactoryCreateDevice:
        return "DeviceFactoryCreateDevice";
    default:
        return "unknown";
    }
}

static void C072AttemptEnableEarlyDebugLayer(C072EntryPoint entry)
{
    const char* const entryName = C072EntryPointName(entry);
    constexpr const char* kNote = "cold_start_experiment/untracked_prior_devices_not_excluded";

    if (State::Instance().currentD3D12Device != nullptr || !State::Instance().d3d12Devices.empty())
    {
        LOG_INFO("[C072] entry={} getDebugInterface_called=false enable_called=false skip_reason=skipped_known_device "
                 "note={}",
                 entryName, kNote);
        return;
    }

    const auto getDebugInterface = D3d12Proxy::D3D12GetDebugInterface_();
    if (getDebugInterface == nullptr)
    {
        LOG_INFO("[C072] entry={} getDebugInterface_called=false enable_called=false "
                 "skip_reason=skipped_null_get_debug_interface_entry note={}",
                 entryName, kNote);
        return;
    }

    ID3D12Debug* debug = nullptr;
    const HRESULT getDebugInterfaceHr = getDebugInterface(IID_PPV_ARGS(&debug));
    if (getDebugInterfaceHr != S_OK || debug == nullptr)
    {
        LOG_INFO("[C072] entry={} getDebugInterface_called=true getDebugInterface_hr=0x{:08X} enable_called=false "
                 "skip_reason=get_debug_interface_failed note={}",
                 entryName, static_cast<uint32_t>(getDebugInterfaceHr), kNote);
        if (debug != nullptr)
            debug->Release();
        return;
    }
    ...
    LOG_INFO("[C072] entry={} getDebugInterface_called=true getDebugInterface_hr=0x{:08X} enable_called=true "
             "skip_reason=none note={}",
             entryName, static_cast<uint32_t>(getDebugInterfaceHr), kNote);
}

static void C072MaybeEnableEarlyDebugLayer(C072EntryPoint entry)
{
    if (!C072DiagEnabledImpl())
        return;

    std::call_once(g_c072EarlyDebugLayerOnce, [entry]() { C072AttemptEnableEarlyDebugLayer(entry); });
}
...
#endif
    C072MaybeEnableEarlyDebugLayer(C072EntryPoint::D3D12CreateDevice);
                              ...
                              REFIID riid, void** ppDevice)
{
    ...
    C072MaybeEnableEarlyDebugLayer(C072EntryPoint::DeviceFactoryCreateDevice);
