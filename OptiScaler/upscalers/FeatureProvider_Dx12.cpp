...
static void ReleaseCreateParams(ContextData<IFeature_Dx12>* contextData)
{
    if (contextData->createParams == nullptr)
        return;

    if (contextData->createParamsOwned)
        TryDestroyNGXParameters(contextData->createParams, NVNGXProxy::D3D12_DestroyParameters());

    contextData->createParams = nullptr;
    contextData->createParamsOwned = false;
}

static void AbortDlssTerminalRebuild(ContextData<IFeature_Dx12>* contextData, UINT handleId, MhwFgFailureStage stage)
{
    contextData->terminalFailure = true;
    MhwFgFailure::Latch(stage);
    ...
    ReleaseCreateParams(contextData);
                                      ...
                                      std::unique_ptr<IFeature_Dx12>* feature)
{
    ...
    const Upscaler requestedUpscaler = upscaler;

    if (MhwFgFailure::PolicyActive() && MhwDlssBackend(requestedUpscaler) && MhwFgFailure::IsLatched())
        return false;
    ...
    case Upscaler::DLSS:
        ...
        else if (MhwFgFailure::PolicyActive())
        {
            MhwFgFailure::Latch(MhwFgFailureStage::ModuleMissing);
            return false;
        }
        else
        {
    ...
    if (!loaded)
    {
        if (MhwFgFailure::PolicyActive() && MhwDlssBackend(requestedUpscaler))
        {
            MhwFgFailure::Latch(MhwFgFailureStage::ModuleMissing);
            return false;
        }
                                         ...
                                         ContextData<IFeature_Dx12>* contextData)
{
    ...
    if (contextData->changeBackendCounter == 1)
    {
        ...
        if (contextData->feature != nullptr)
        {
            ...
            contextData->createParamsOwned = !isPassthrough;
    ...
    if (contextData->changeBackendCounter == 2)
    {
        ...
        if (!GetFeature(state.newBackend, handleId, contextData->createParams, &contextData->feature))
        {
            ...
            if (MhwFgFailure::PolicyActive() && MhwDlssBackend(state.newBackend))
            {
                AbortDlssTerminalRebuild(contextData, handleId, MhwFgFailureStage::FeatureCreate);
                return false;
            }
    ...
    if (contextData->changeBackendCounter == 3)
    {
        ...
        if (!initResult)
        {
            ...
            if (MhwFgFailure::PolicyActive() && MhwDlssBackend(state.newBackend))
            {
                AbortDlssTerminalRebuild(contextData, handleId, MhwFgFailureStage::RebuildFailed);
                return false;
            }
